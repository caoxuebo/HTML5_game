...
var SmokingHole = function(numSmokeBubbles, numFireParticles, left, top, width) {
	...
	this.MAX_DELAY = 6000; // Max time between eruptions
	...
	// The smokeBubbleCursor is an index into the smokingBubble array
...
SmokingHole.prototype = {
	...
	update: function(now, fps, context, lastAnimationFrameTime) {
		...
		this.updateSmokeParticles(now, fps, context, lastAnimationFrameTime);
		
		// All bubbles are revealed when smokeBubbleCursor is zero
		if (this.smokeBubbleCursor !== 0 && this.eruptionTimer.isExpired()) {
			this.revealSmokeBubble();
		}
	},
	
	updateFireParticles: function(now, fps, context, lastAnimationFrameTime) {
		for (var i = 0; i < this.fireParticles.length; ++i) {
			if (this.fireParticles[i].visible) {
				this.fireParticles[i].sprite.update(now, fps, context, lastAnimationFrameTime);
			}
		}
	},
	
	updateSmokeParticles: function(now, fps, context, lastAnimationFrameTime) {
		for (var i = 0; i < this.smokeBubbles.length; ++i) {
			if (this.smokeBubbles[i].visible) {
				this.smokeBubbles[i].sprite.update(now, fps, context, lastAnimationFrameTime);
			}
		}
	},
